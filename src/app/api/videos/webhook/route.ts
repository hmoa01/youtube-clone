...
export const POST = async (request: Request) => {
  ...
  switch (payload.type as WebhookEvent["type"]) {
    ...
    case "video.asset.ready": {
      ...
      if (existingVideo[0]?.thumbnailUrl && existingVideo[0]?.previewUrl) {
        await db
          .update(videos)
          .set({
      ...
      // duration is in seconds, so we need to multiply by 1000 to get milliseconds
    ...
    case "video.asset.track.ready": {
      ...
      // Typescript doesn't know that asset_id is always present in the data object
      // so we need to use a type assertion to tell it that it is
